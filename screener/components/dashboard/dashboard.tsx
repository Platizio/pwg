 ...
 *
 * Every figure arrives as one snapshot from the server. Nothing below reads a
 * clock or a feed of its own: the tree under app/(terminal)/layout.tsx is a
 * client tree, and a second source of time down here is a hydration failure.
 */
            ...
            {shown.map((group) => (
              <Card key={group.name} className="flex flex-col px-5 py-5">
                ...
                <ul className="m-0 mt-4 mb-5 flex list-none flex-col gap-0.5 p-0">
                  {group.members.slice(0, PER_SECTOR).map((q) => {
                    const row = (
                      <>
                ...
                <Link
                  href={`/sector/${sectorSlug(group.name)}`}
    ...
    <>
      ...
      <Reader
        ...
        footer={
          openEvent?.ticker && (
            <Link
              href={`/instrument/${openEvent.ticker}`}
      ...
      <Reader
        ...
        footer={
          openNews && (
            <Link
              href={`/instrument/${openNews.ticker}`}
              className="flex min-h-11 items-center gap-2 text-[13px] font-medium text-gold transition-colors hover:text-gold-hi"
            >
