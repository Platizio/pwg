...
export function Sidebar({
  ...
  const ticker = /^\/instrument\/([^/]+)/.exec(pathname)?.[1];
  const current = ticker ? decodeURIComponent(ticker) : null;

  const nav = [
    { key: "overview", label: "Overview", Icon: IconOverview, href: "/", active: pathname === "/" },
    {
      key: "watchlist",
      label: "Watch list",
      Icon: IconWatchlist,
      href: `/instrument/${current ?? DEFAULT_TICKER}`,
      active: current !== null,
    },
    {
      key: "calendar",
  ...
  return (
    ...
    >
      ...
      <section
        ...
        className={cn("mt-7 min-h-0 pb-8", collapsed && "w-full")}
      >
        <p
          id="watchlist-label"
                    ...
                    {!collapsed &&
                      (quotes?.[s.id] === undefined ? (
                        ...
                        <span
                          ...
                          style={{ color: quotes[s.id] >= 0 ? C.up : C.down }}
                        >
                          {pct(quotes[s.id])}
                        </span>
                      ))}
