...
test("toSectorGroups returns all eleven sectors, strongest day first", () => {
  ...
  assert.equal(groups.at(-1)?.name, "Information technology");
  for (let i = 1; i < groups.length; i += 1) {
    assert.ok(groups[i - 1].day >= groups[i].day, "sectors are not in descending day order");
  }
