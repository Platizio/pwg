 ...
 * read the same thing twice. `loading` and `decoding` are on the two secondary
 * cards only; the first is the largest contentful paint and must not be
 * deferred.
 */
        ...
        <div className="container">
          ...
          <div className="topic-grid">
            {TOPICS.map((topic) => {
              ...
              return (
                <Link className="topic-card" to={`/articles/topic/${topic.id}`} key={topic.id}>
                  <h3>{topic.title}</h3>
                  <p>{topic.blurb}</p>
