...
/** Shorter on fallback, so a transient outage isn't pinned for half a day.
    Only for paths where NO search was billed — see CACHE_SPENT. */
const CACHE_FALLBACK = 'public, s-maxage=3600, stale-while-revalidate=86400'
/**
 * For fallbacks that happen AFTER a search was already billed against the
 * 2000-request lifetime pool.
 *
 * A thin result and an exhausted quota both still cost the search that
 * discovered them, so pinning them for an hour spends the pool at 24 calls a
 * day rather than the two the note above budgets for — the quota would be gone
 * in about three months, silently, because the curated rail renders identically
 * either way. Cached as long as a live answer: we have already paid, and an
 * hour later the upstream is unlikely to have a different answer.
 */
const CACHE_SPENT = CACHE_LIVE
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (body?.error) {
      ...
      // The pool never refills, so retrying hourly buys nothing.
      console.error('[api/news] upstream error field set; serving curated')
      return json(curatedPayload(), 200, CACHE_SPENT)
    }
