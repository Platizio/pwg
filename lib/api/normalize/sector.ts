  ...
  return staged
    .map(({ name, etf, members, turnoverM, membersChg }) => ({
      ...
      /* changePercent arrives as a fraction (-0.0149 = -1.49%).
         ...
         Null rather than 0 when the fund did not quote: `?? 0` printed a green
         "+0.00%" on the card and, worse, sorted that invented flat reading into
         the middle of the strip as though it had been measured. */
      day: (() => {
        const q = quoteByEtf.get(etf);
        return q && !q.notFound && typeof q.changePercent === "number"
          ? q.changePercent * 100
          : null;
      })(),
      week: weekByEtf.get(etf) ?? null,
    ...
    /* Strongest sector first, as SECTORS was ordered: the interface does the
       ...
       A sector whose fund did not quote has not underperformed — it is unknown,
       and it sinks to the bottom rather than being ranked as flat. */
    .sort((a, b) => {
