...
export type SectorSnapshot = {
  ...
  slug: string;
  fund: string;
  /* Null when the tracking fund did not quote. Zero is a real flat day, so the
     two cannot share a value — see the assignment for what went wrong when they
     did. */
  day: number | null;
  week: number | null;
  stocks: SectorRow[];
  ...
  async (slug: string): Promise<SectorSnapshot | null> => {
    ...
    return {
      ...
      /* A raw quote, so changePercent is still a fraction.
         ...
         `?? 0` used to stand in here, which turned a fund that failed to quote
         into a green "+0.00%" under a sentence asserting it was the fund's move
         — a fabricated figure wearing a real sector's name, on the one page
         built to explain that sector. Absence and a flat day are different
         facts. `week` eight lines up already made that distinction, and
         index-proxy.ts enforces it for the index strip; this now matches both. */
      day:
        fundQuote && !fundQuote.notFound && typeof fundQuote.changePercent === "number"
          ? fundQuote.changePercent * 100
          : null,
      week,
      stocks,
      funds,
